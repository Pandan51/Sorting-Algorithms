...
namespace Sorting_Algorithms
{
    internal class Radix_Sort<T> where T : IComparable<T>
    {
        ...
        /// </summary>
        public static void Sort(T[] data, Clock clock)
        {
        ...
        private static void SortInt(int[] arr, Clock clock)
        {
            ...
            for (long exp = 1; (maxShifted / exp) > 0 && !clock.IsElapsed(); exp *= 10)
            {
                ...
                // Update UI (Throttled inside ProgressUI)
                int currentDigit = (int)Math.Log10(exp) + 1;
                int totalDigits = (int)Math.Log10(maxShifted) + 1;
                //ProgressUI.Update(currentDigit, totalDigits, clock);
            }
        }
        ...
        private static void SortString(string[] arr, Clock clock)
        {
            ...
            for (int d = maxLen - 1; d >= 0 && !clock.IsElapsed(); d--)
            {
                ...
                //ProgressUI.Update(maxLen - d, maxLen, clock);
            }
        }
